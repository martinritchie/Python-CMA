...
def CMA(D, subgraphs, subgraphSeq):
    ...
    # 2. Each subgraph sequence must sum to a number that is divisible by the #    number of nodes in the corresponding subgraph.
    # ---------------------------- - Output - --------------------------------
    # A: a networkx graph. 
    # ------------------------ - Example call - ------------------------------
    # See CMAtestCases.py for various examples. 
    # ----------------------------- - Notes - --------------------------------
    # For incomplete subgraphs the subgraphs must be enumerated so that corners of the same type are represented by adjacent integers see the following toast for example:  
    #       toast = subGraph([(0, 2), (0, 3), (2, 3), (1, 2), (1, 3)])
    # The ordering is essential to the connection process. 
    # ------------------------- - Key variables - ----------------------------
    N = len(D)
    d = list(D) 
    # hStubTup, hyperstub tuples. Each element of the list, hStubTup, is a  sequence of length N of tuples(including 1-tuples). There is one list of tuples per subgraph. The tuples size corresponds to the number of corner types of the sequences' subgraphs. The value of these tuples is decided multinomially for 2-tuples or greater. The values represnt the classical stub count of the hyperstub. The tuples have not yet been allocated to subgraphs.
    hStubTup = []

    # tupInducedD, tuple induced degree. Each element of the list, tupInducedD, is an integer sequence. There is one such sequence per subgraph. Each sequence is computed by summing the tuples in the assoicated
    # hStubTup sequence. This sum denotes minimumn degree required for a
    # particular hyperstub configuration.
    tupInducedD = []

    # sSeq, subgraph sequences. A list of sequences contain the corner       count(s) for a particular subgraph. For incomplete subgraphs this will be tuple. 
    sSeq = []
    
    # OsSeq, ordered subgraph sequences. An ordered version of sSeq, i.e, index i of a particular sequence corresponds to the corner types incident to node i. 
    OsSeq = []
    # H, hyperstub sequences. The same as OsSeq but the counts are multipled by thier stub cardinality. 
    H = []
